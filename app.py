...
from flask import Flask ,render_template ,flash,redirect,request , session  #flask-framework ,#Flask-python class
    ...
    #handle form data
     ...
     if request.method=='POST': #check if user posted something
         ...
         if file=="":
             ...
             sql= "INSERT INTO `books`(`title`,`author`,`description`, `file`, `genre`) VALUES (%s,%s,%s, %s, %s)"  # %s protects data .
             try:


                cursor.execute(sql,(title,author,description,filename,genre))
